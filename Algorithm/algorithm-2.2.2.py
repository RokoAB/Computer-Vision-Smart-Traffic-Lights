...
log_file = "object_detection_log2.csv"
...
while True:
    ...
    for ref_frame, new_frame in zip(last_green_samples, last_red_samples):
        ...
        margin_y = int(frame_height * 0.15)
        central_roi = (margin_x, margin_y, frame_width - 2 * margin_x, frame_height - 2 * margin_y)
        height, width = frame_gray.shape
        center_x, center_y = width // 2, height // 2
        margin_x, margin_y = width // 4, height // 4  # Keep center 50% width/height
        for cnt in contours:
            ...
            # Filter out unusually shaped objects (very flat or tall ones)
            if aspect_ratio < 0.3 or aspect_ratio > 3.0:
                continue

            object_count += 1
            object_sizes.append(int(area))

            cv2.rectangle(frame_color, (x, y), (x + w, y + h), (255, 255, 255), 2)
            cv2.putText(frame_color, f"Obj #{object_count}", (x, y - 10),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)


        cv2.rectangle(frame_color,
                      (central_roi[0], central_roi[1]),
                      (central_roi[0] + central_roi[2], central_roi[1] + central_roi[3]),
                      (0, 255, 255), 1)
        ...
        cv2.imshow("Reference Frame (Green)", cv2.resize(ref_frame, (320, 240)))
        cv2.imshow("Comparison Frame (Red)", cv2.resize(new_frame, (320, 240)))
        cv2.imshow("Difference / Motion Mask", cv2.resize(thresh, (320, 240)))
