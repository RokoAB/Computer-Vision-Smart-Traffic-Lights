...
const unsigned long redBuffer = 4000;

bool extendNSGreen = false;
bool extendEWGreen = false;

bool startupSyncSent = false;
...
void setup() {
  ...
  // Send startup sync signal
  while (!Serial) ;  // Wait for Serial to be ready (especially useful when connected to Pi/laptop)
  delay(1000);       // Small delay for devices to boot
  Serial.println("SYNC_START");  // 🔁 Message to signal system start
  startupSyncSent = true;
}

void loop() {
  // === NORTH-SOUTH GREEN PHASE ===
  sendGreenCommand("NS", extendNSGreen);
  ...
  sendGreenCommand("EW", extendEWGreen);
...
void sendGreenCommand(String dir, bool extend) {
  // Example: GREEN_NS:DEFAULT or GREEN_EW:EXTENDED
  Serial.print("GREEN_");
  Serial.print(dir);
  Serial.print(":");
  Serial.println(extend ? "EXTENDED" : "DEFAULT");
}
